...
DIR=result/TEST-SENS30-ALL3
OUTPUT=run-sens30-all3
LEN=1
#LEN=10
...
for idx in "${!OP_SET[@]}"; do
	...
	rm -rf $DIR/$OP
	mkdir -p $DIR/$OP
	$SCRIPT/test-gen-eval-all.sh $DIR/$OP -rep $LEN -mut 20 20.1 1 -ozr $OZ_OPT -zor $ZO_OPT -mvr $MV_OPT -locus $LOC_OPT -sample $SAMPLE_OPT -clone 200 200 5 -step 4 4 1 -gen synth -aic 0 -adc 1 -akc 0 -run I -opt-gen '--treeGrowFilterOutChildSize 2 --samplingMethod uniform' &>$DIR/$OP/$OUTPUT.eo
...
ls $DIR/MV | grep '^[0-9]*$' | while read CNT; do
#for ((CNT=1; CNT<=11; CNT++)); do 
for ((RCNT=0; RCNT<$LEN; RCNT++)); do
	...
	for b in impute true ; do
		...
		python $SCRIPT/clone-tree-to-mu-tree-imput.py $LDIR/$b-tree.txt $LDIR/$b-clones.txt $LDIR/input-scite.txt $LDIR/mutation-info.txt $LDIR/cell-names.txt $LDIR/output-$b-mut2 
		#--compress 
