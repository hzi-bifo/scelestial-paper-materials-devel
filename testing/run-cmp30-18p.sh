...
mkdir $DIR/$REP
#sem -j $LEN 
$SCRIPT/test-gen-eval-all.sh $DIR/$REP -rep 1 -mut 2 2.1 1 -ozr $OZ_OPT -zor $ZO_OPT -mvr $MV_OPT -locus $LOC_OPT -sample $SAMPLE_OPT -clone 5 10 5 -step 5 5 1 -gen onco -run2 $RUN2 -keep -seed $((77777*REP)) -timeout 10h 36000 &>$DIR/$REP/$OUTPUT-$REP.eo
done
sem --wait
...
&\\multicolumn{2}{|c|}{150 samples} & \\multicolumn{2}{|c|}{200 samples} & \\multicolumn{2}{|c|}{250 samples} \\\\
& \\multicolumn{1}{|c|}{5 clones} & \\multicolumn{1}{|c|}{10 clones} & \\multicolumn{1}{|c|}{5 clones} & \\multicolumn{1}{|c|}{10 clones} & \\multicolumn{1}{|c|}{5 clones} & \\multicolumn{1}{|c|}{10 clones} \\\\
Method & 50 sites & 50 sites & 50 sites & 50 sites & 50 sites & 50 sites\\\\ \\hline "
cat $DIR/$OUTPUT.txt | python $SCRIPT/calculate-average.py --samples 150 200 250 --clones 5 10 --sites 50 | sed -e "1d"
echo "\\end{tabular}"
)> $DIR/$OUTPUT-avg.txt
