#!/bin/bash
source init.sh
DIR=result/TEST-CMP30-3b2s/
LEN=1
OUTPUT=run-cmp30-3b2s

for ((i=0; i<=9; i++)); do sem -j 20 --id cmp30-3b2 ./run-cmp30-3b2sXX.sh $i; done
sem --wait --id cmp30-3b2

OUTPUT=run-cmp30-3b2
(for ((i=0; i<=9; i++)); do cat result/TEST-CMP30-3b2s/$i/run-cmp30-3b2s$i.eo | grep -v "Done TEST-GEN-AL"; done; echo "Done TEST-GEN-ALL"; ) > $DIR/$OUTPUT.eo
