...
#Running beast
		if [[ $RUN =~ .*E.* || $RUN2 =~ .*,beast,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-beast.sh $DIR $SAMPLE $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
# Running OncoNEM
		if [[ $RUN =~ .*O.* || $RUN2 =~ .*,onco,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-onconem.sh $DIR $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
# Running BitPhylogeny
		if [[ $RUN =~ .*B.* || $RUN2 =~ .*,bitphyl,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-bp.sh $DIR $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
#Running SCITE
		if [[ $RUN =~ .*S.* || $RUN2 =~ .*,scite,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-scite.sh $DIR $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
#Running BSC [Beast Single-cell Plugin]
		if [[ $RUN =~ .*U.* || $RUN2 =~ .*,scplug,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-bsc.sh $DIR $BSCFOLDER $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
#Running BSC [Beast Single-cell Plugin]
		if [[ $RUN =~ .*L.* || $RUN2 =~ .*,scplugwl,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-bsc-loc.sh $DIR $BSCFOLDER $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
#Running phiscs
		if [[ $RUN2 =~ .*,phiscs,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-phiscs.sh $DIR 600 $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
#Running sasc
		if [[ $RUN2 =~ .*,sasc,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-sasc.sh $DIR $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
#Running sciphi
		if [[ $RUN2 =~ .*,sciphi,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-sciphi.sh $DIR $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
#Running sifit
		if [[ $RUN2 =~ .*,sifit,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-sifit.sh $DIR 10000 $MATRIX_DISTANCE_NORMALIZATION 2>&1
...
#Running siclonefit
		if [[ $RUN2 =~ .*,siclonefit,.* ]]; then
			timestamp
			$SEM $TIMEOUT $SCRIPT/run-siclonefit.sh $DIR 1 $MATRIX_DISTANCE_NORMALIZATION 2>&1
	...
	for b in 'impute' 'bp' 'onconeme' 'sasc' 'sciphi' 'scite' 'siclonefit' 'sifit' ; do
		if [ -f $DIR/$b-tree.txt ]; then
			echo $a $b:
			echo -e ""
			echo python $SCRIPT/visualize-tree.py $b $DIR/$b-tree.txt $DIR/$b-clones.txt $DIR/output-$b --type-file $DATA_TYPE --cell-name $DIR/cell-names.txt --color $DIR/colors.txt 
