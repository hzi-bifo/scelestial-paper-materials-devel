...
source init.sh
DIR=result/TEST-CMP30-17-OS
OUTPUT=run-cmp30-17-os
LEN=1
DIRM=result/TEST-CMP30-17
OUTPUTM=run-cmp30-17
...
cat $DIR/$OUTPUT.eo | python $SCRIPT/extract-log-results.py --rep $LEN --print "^DIR=" "Done TEST-GEN-ALL" --pat1 "DIR=" "SAMPLE=" sample "LOCUS=" site "STEP=" step "MVR=" mvr "0->1=" zor "1->0:" ozr "STEPMUT:" stepmut "CLONE:" clone --pg1 "^True? [0-9]+" "\\s T?" "\\s I?" --pg2 "^pTrue? [0-9]+" "\\s pT?" "\\s pI?" --sg1 '^([0-9.]*)user[ \t]*(\S+)' dimpt?  --sg1-fault RUN_END | python $SCRIPT/eq-to-csv-2.py -mean-na nan --len $LEN -a I? impt vimpt -a pI? pimpt vpimpt -a dimpt? dimpt vdimpt > $DIR/$OUTPUT.txt



python bin/remove-column.py I0,dimpt,dimpt0,impt,pI0,pimpt,vimpt,vpimpt,vdimpt < $DIR/$OUTPUT.txt > $DIR/$OUTPUT-rm.txt
paste $DIR/$OUTPUT-rm.txt $DIRM/$OUTPUTM.txt | python $SCRIPT/calculate-average.py > $DIR/$OUTPUT-avg.txt
