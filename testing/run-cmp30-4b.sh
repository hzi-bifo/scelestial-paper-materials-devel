#!/bin/bash
source init.sh
DIR=result/TEST-CMP30-4bs/
LEN=1
OUTPUT=run-cmp30-4bs

for ((i=0; i<=9; i++)); do sem -j 20 --id cmp30-4b ./run-cmp30-4bsXX.sh $i; done
sem --wait --id cmp30-4b

OUTPUT=run-cmp30-4b
(for ((i=0; i<=9; i++)); do cat result/TEST-CMP30-4bs/$i/run-cmp30-4bs$i.eo | grep -v "Done TEST-GEN-AL"; done; echo "Done TEST-GEN-ALL"; ) > $DIR/$OUTPUT.eo
