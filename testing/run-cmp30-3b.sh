#!/bin/bash
source init.sh
DIR=result/TEST-CMP30-3bs/
LEN=1
OUTPUT=run-cmp30-3bs

for ((i=0; i<=9; i++)); do sem -j 20 --id cmp30-3b ./run-cmp30-3bsXX.sh $i; done
sem --wait --id cmp30-3b

OUTPUT=run-cmp30-3b
(for ((i=0; i<=9; i++)); do cat result/TEST-CMP30-3bs/$i/run-cmp30-3bs$i.eo | grep -v "Done TEST-GEN-AL"; done; echo "Done TEST-GEN-ALL"; ) > $DIR/$OUTPUT.eo
